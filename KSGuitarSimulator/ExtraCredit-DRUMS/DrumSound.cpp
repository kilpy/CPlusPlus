...
#include <random>
//boop
DrumSound::DrumSound(double frequency){	//create a guitar string sound of the given
...
DrumSound::DrumSound(vector<int16_t> init)	//create a guitar string with size
...
void DrumSound::pluck(void)		//replace the buffer with random values
{
...
void DrumSound::tic(void){		//advance the simulation one time step
...
int16_t DrumSound::sample(void){	//return the current sample
return buffer->peek();
}
int DrumSound::time(void){		//return number of times tic was called
return count;
}
